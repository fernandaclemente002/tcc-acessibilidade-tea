...
const SelecionarLista = ({ navigation }) => {
    const [lista, setLista] = useState([]);
    const [loading, setLoading] = useState(true);

    const fetchLista = async () => {
        try {
            const response = await axios.get("http://localhost:3000/Listas");
    ...
    const renderItem = ({ item }) => (
        <View style={styles.listaItem}>
            <TouchableOpacity onPress={() => [guardaLista(item.TB_LISTA_NOME), navigation.navigate("CatPro")]}>
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <Text style={styles.title}>Escolher Lista</Text>
            ...
            <View style={styles.content}>
                {loading ? (
                    ...
                    <>
                        <Text style={styles.text}>Ainda não existem listas...</Text>
                        <TouchableOpacity
